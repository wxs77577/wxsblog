...
var deskper=dwidth/dheight;
function Pic(urls,w,h){
	ic.Url = urls;
	ic.Init();
	res(w,h)
}
...
if((wallwidth/wallheight)>=deskper){
	dw=wallheight*deskper; dh=wallheight
	}
else{
	 dw=wallwidth; dh=wallwidth/deskper
	}
	...
	ic.dragHeight = dh-2;
	ic.Init();
	document.getElementById("sbn").value="生成 "+w+"x"+h+" 格式图片或壁纸"
	document.getElementById("newsize").innerHTML=w+"x"+h+""
	//document.getElementById("sbn").style.border="1px solid #dedede"
	//document.getElementById("sbn").style.background="#008000"
	//document.getElementById("sbn").style.color="#fff"
}
...
function crop(){
	...
	if(sbn!="请先选择处理格式"){
	document.getElementById("deskwidth").value=dwidth
	document.getElementById("deskheight").value=dheight
	document.getElementById("bigurl").value=bigurl
	document.getElementById("smallwidth").value=wallwidth
	document.getElementById("smallheight").value=wallheight
	//document.f.submit()
	}
	else{
		alert("请先选择壁纸格式尺寸 ! ")
...
Drag.prototype = {
  ...
  Move: function(oEvent) {
	...
	this._obj.style.top = iTop + "px";	
	    document.getElementById("leftpos").value=iLeft
	    	document.getElementById("toppos").value=iTop
	//附加程序
	this.onMove();
  },
  //停止拖动
...
ImgCropper.prototype = {
  ...
  PreView: function() {
	...
	newpw=(w)
	newph=( h)
	document.getElementById("viewDiv").style.width=newpw+"px"
	document.getElementById("viewDiv").style.height=newph+"px"
  }
}
...
if(wall.complete||wall.error){
clearTimeout(tt)
//wallwidth=wall.width;wallheight=wall.height
wallwidth=450;wallheight=(wall.height*450)/wall.width
var oww=wall.width;var owh=wall.height
var pper=wallwidth/wallheight
				if(wallwidth>=wallheight){
								if((wallwidth/wallheight)>=deskper){
									dw=wallheight*deskper; dh=wallheight
									}
 ...
 ic = new ImgCropper("bgDiv", "dragDiv", wall.src, wallwidth, wallheight, {
